...
<aside
	class="flex min-w-0 flex-col h-full bg-bg-panel overflow-hidden"
...
>
	...
	<!-- Body -->
	<div
		class="flex-1 overflow-hidden transition-opacity duration-200"
		class:opacity-0={!isOpen}
		class:opacity-100={isOpen}
		style="transition-delay: {isOpen ? '150ms' : '0ms'}"
	>
		{#if tool === 'timer'}
			<TimerTool />
		{:else if tool === 'notes'}
